...
GUIWindow_Spellbook::GUIWindow_Spellbook() : GUIWindow(WINDOW_SpellBook, {0, 0}, { 640, 480 }) {
...
void GUIWindow_Spellbook::openSpellbook() {
    ...
    for (SpellId spell : spellsForMagicSchool(chapter)) {
        ...
        CreateButton(fmt::format("SpellBook_Spell{}", index),
                     {pViewport->uScreen_TL_X + pIconPos[chapter][pSpellbookSpellIndices[chapter][index + 1]].Xpos,
                     pViewport->uScreen_TL_Y + pIconPos[chapter][pSpellbookSpellIndices[chapter][index + 1]].Ypos},
                ...
                if (player.bHaveSpell[spell] || engine->config->debug.AllMagic.value()) {
                    ...
                    if (SBPageSSpellsTextureList[index + 1]) {
                        ...
                        if (pTexture) {
                            ...
                            pX_coord = pViewport->uScreen_TL_X + iconPos.Xpos;
                            pY_coord = pViewport->uScreen_TL_Y + iconPos.Ypos;
